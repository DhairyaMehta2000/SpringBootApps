	...
	public void play() {
		...
		while (true) {
			...
			if (makeMove(move, current_player, end_pos)) {
				...
				for (int i = 0; i < ladders.ladders.size(); i++) {
					if (ladders.ladders.get(i) == end_pos) {
						System.out.println(
								"Player " + current_player.getName() + " got ladder at " + ladders.ladders.get(i));
						got_ladder = true;
						ladders.promote(current_player, end_pos);
					}
				}
//				

				if (!got_ladder) {
					for (int i = 0; i < snakes.snakes.size(); i++) {
						if (snakes.snakes.get(i) == end_pos) {
							System.out.println(
									"Player " + current_player.getName() + " got snake at " + snakes.snakes.get(i));
